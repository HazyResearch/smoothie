"""
This script implements the labeled kNN baseline for the multi-model setting.
"""
...
def run_labeled_knn(args, data_config, model_group_name, model_group, embedder):
    ...
    output_fpath = construct_labeled_knn_predictions_path(
        data_config=data_config, model_group_name=model_group_name, args=args
    )
    ...
    final_generations = (
        []
    )  # Final generations. Shape: (args.label_train_n_trials, n_test)
...
def main(args):
    ...
    if args.multi_model:
        ...
        run_labeled_oracle(args=args, data_config=data_config, model_group="")

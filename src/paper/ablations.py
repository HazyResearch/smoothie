    ...
    for model_group in ["3", "7"]:
        ...
        all_scores = {
            dataset: {
                ...
                for metric in ["random", "test", "train", "oracle"]
            }
            for dataset in datasets
        }
        ...
        for metric, metric_name in [
            ...
            score_str = " & ".join(
                [
                    ...
                    if metric in ["random", "test", "train"]
...
def plot_smoothie_neighborhood_size_ablation(plot_output_path=None):
    ...
    for model_group in model_groups:
        ...
        for dataset in datasets:
            ...
            for method, neighborhood_size in zip(smoothie_methods, neighborhood_sizes):
                ...
                results[model_group][dataset][
                    f"smoothie_{neighborhood_size}"
                ] = np.round(smoothie_score * 100, 1)
    ...
    for dataset in datasets:
        ...
        for model_group in model_groups:
            neighborhood_scores = [
                results[model_group][dataset][f"smoothie_{size}"]
        ...
        plt.title(
            f"Effect of Neighborhood Size\n({dataset2name[dataset]})",
            pad=PLOT_TITLE_PAD,
            fontweight=PLOT_TITLE_FONTWEIGHT,
        )
        plt.xlabel(
            "$n_0$", labelpad=PLOT_AXIS_LABEL_PAD, fontweight=PLOT_AXIS_LABEL_FONTWEIGHT
        )
        plt.ylabel(
            "Score", labelpad=PLOT_AXIS_LABEL_PAD, fontweight=PLOT_AXIS_LABEL_FONTWEIGHT
        )
        ...
        plot_file_path = (
            Path(plot_output_path).parent
            / f"smoothie_neighborhood_size_ablation_{dataset}.png"
        )
...
compute_smoothie_train_test_selection_results_latex_table(
    table_output_path="tables/smoothie_train_test_selection_comparison.tex"
)
compute_smoothie_train_test_routing_results_latex_table(
    table_output_path="tables/smoothie_train_test_routing_comparison.tex"
)
plot_smoothie_neighborhood_size_ablation(
    plot_output_path="plots/smoothie_neighborhood_size_ablation.png"
)
compute_smoothie_embedding_ablation_latex_table(
    table_output_path="tables/smoothie_routing_embedding_ablation.tex"
)
compute_smoothie_global_many_ensembles_plot(
    plot_output_path="plots/smoothie_global_many_ensembles.png"
)

...
def compute_alpaca_selection_results(plot_output_path=None):
    ...
    pick_random_lc_win_rates = [
        0
    ] * 10  # ith entry is lc-win rate of pick_random_trial_num=i
    smoothie_win_rates = [
        0
    ] * 10  # ith entry is win rate of smoothie_independent_all-mpnet-base-v2_trial_num=i
    smoothie_lc_win_rates = [
        0
    ] * 10  # ith entry is lc-win rate of smoothie_independent_all-mpnet-base-v2_trial_num=i
    for idx in range(len(results_df)):
        method = results_df.iloc[idx]["method"]
        if method in ALPACA_MODELS:
            model_win_rates[method] = results_df.iloc[idx]["win_rate"]
            model_lc_win_rates[method] = results_df.iloc[idx][
            ...
            pick_random_lc_win_rates[trial_num] = results_df.iloc[idx][
                "length_controlled_winrate"
            ]
            ...
            smoothie_lc_win_rates[trial_num] = results_df.iloc[idx][
                "length_controlled_winrate"
            ]

    # Calculate differences
    win_rate_diffs = [
        smoothie_win_rates[i] - pick_random_win_rates[i] for i in range(10)
    ]
    lc_win_rate_diffs = [
        smoothie_lc_win_rates[i] - pick_random_lc_win_rates[i] for i in range(10)
    ]
    ...
    create_box_and_whisker_plot(
        ...
        title="Smoothie vs\nRandom on Alpaca",
        ylabel="Win Rate\nImprovement",
        plot_output_path=plot_output_path.replace(".png", "_win_rate_diff.png")
        if plot_output_path
        else None,
    ...
    create_box_and_whisker_plot(
        ...
        title="Smoothie vs\nRandom on Alpaca",
        ylabel="LC Win Rate\nImprovement",
        plot_output_path=plot_output_path.replace(".png", "_lc_win_rate_diff.png")
        if plot_output_path
        else None,
        yticks=[-10, 0, 10, 20, 30],
    )
    ...
    for i in range(10):
        ...
        fpath = Path(
            f"smoothie_data/alpaca/algorithm_outputs/smoothie_independent_all-mpnet-base-v2_{i}_n_neighbors=1_trial_num={i}.json"
        )
        ...
        sorted_models_wr = sorted(
            ensemble, key=lambda x: model_win_rates[x], reverse=True
        )
        if sorted_models_wr[0] == model_selected:
            selected_best_by_win_rate += 1

        # Count if Smoothie selects best model (by lc-win-rate)
        sorted_models_lcwr = sorted(
            ensemble, key=lambda x: model_lc_win_rates[x], reverse=True
        )
        ...
        spearman_corr = stats.spearmanr(
            smoothie_weights, ensemble_win_rates
        ).correlation
...
def compute_smoothie_nlg_results(plot_output_path=None):
    ...
    for model_group in model_groups:
        ...
        for dataset in datasets:
            ...
            model_scores = [
                scores["ensemble"][model] for model in MODEL_GROUPS[model_group]
            ]

            # Compute smoothie test correlations
            generations_fpath = (
                RESULTS_DIR / dataset / model_group / f"{smoothie_test}_test.json"
            )
            smoothie_weights = json.loads(generations_fpath.read_text())[
                "smoothie_weights"
            ][0]
    ...
    print(
        f"Number of correlations below 0: {np.sum(np.array(smoothie_test_correlations) < 0)}/{len(smoothie_test_correlations)}"
    )
...
def compute_smoothie_latex_table(table_output_path=None):
    ...
    latex_content.extend(
        [
            "\\toprule",
            "& & "
            + " & ".join([dataset2name.get(d, d.title()) for d in datasets])
            + " \\\\ \\midrule",
        ]
    )
    ...
    for model_group in ["3", "7"]:
        ...
        all_scores = {
            dataset: {
                metric: results[model_key][dataset][metric]
                for metric in ["random", "test", "oracle"]
            }
            for dataset in datasets
        }
        ...
        best_unsupervised = {
            dataset: [
                m
                for m in ["random", "test"]
                if all_scores[dataset][m]
                == max(all_scores[dataset][m] for m in ["random", "test"])
            ]
            for dataset in datasets
        }
        best_overall = {
        ...
        for metric, metric_name in [
            ...
            score_str = " & ".join(
                [
                    f"\\underline{{\\textbf{{{score:.1f}}}}}"
                    if metric in ["random", "test"]
                    and metric in best_unsupervised[dataset]
                    and metric in best_overall[dataset]
                    else f"\\underline{{{score:.1f}}}"
                    if metric in ["random", "test"]
                    and metric in best_unsupervised[dataset]
                    else f"\\textbf{{{score:.1f}}}"
...
def compute_mix_instruct_correlations():
    ...
    smoothie_test = "smoothie_sample_independent_test_time_all-mpnet-base-v2"
    generations_fpath = (
        RESULTS_DIR / "mix_instruct" / "mi_all" / f"{smoothie_test}_test.json"
    )
...
def create_box_and_whisker_plot(
    data, labels, title, ylabel, plot_output_path=None, ylim=None, yticks=None
):
    """
    ...
    ax.set_title(title, fontweight=PLOT_TITLE_FONTWEIGHT, pad=PLOT_TITLE_PAD)
    ax.set_ylabel(
        ylabel, fontweight=PLOT_AXIS_LABEL_FONTWEIGHT, labelpad=PLOT_AXIS_LABEL_PAD
    )
...
def compute_smoothie_global_ensembles_plot(plot_output_path=None):
    ...
    plt.title(
        f"Rank of Model Selected by Smoothie-Global",
        pad=PLOT_TITLE_PAD,
        fontweight=PLOT_TITLE_FONTWEIGHT,
    )

    plt.xlabel(
        "Rank", labelpad=PLOT_AXIS_LABEL_PAD, fontweight=PLOT_AXIS_LABEL_FONTWEIGHT
    )
    plt.ylabel(
        "Frequency", labelpad=PLOT_AXIS_LABEL_PAD, fontweight=PLOT_AXIS_LABEL_FONTWEIGHT
    )
...
compute_smoothie_nlg_results(
    plot_output_path="plots/smoothie_nlg_weight_correlations.png"
)
compute_smoothie_latex_table(
    table_output_path="tables/smoothie_nlg_selection_table.tex"
)
compute_mix_instruct_correlations()
compute_smoothie_global_ensembles_plot(
    plot_output_path="plots/smoothie_global_nlg_ensembles.png"
)

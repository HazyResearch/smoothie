...
def compute_smoothie_latex_table(table_output_path=None):
    ...
    for model in models:
        ...
        print(f"Smoothie-global selects the best model {best_model_selections} times for {model}.")
    ...
    latex_content.extend([
        "\\toprule",
        "& & " + " & ".join([f"\\small{{{dataset2name.get(d, d.title())}}}" for d in datasets]) + " \\\\ \\midrule"
    ])
    ...
    for model in models:
        ...
        all_scores = {dataset: {metric: results[model][dataset][metric] for metric in ["random", "global", "local", "oracle"]} for dataset in datasets}
        
        # Determine the best unsupervised and overall methods for each dataset
        best_unsupervised = {dataset: [m for m in ["random", "global"] if all_scores[dataset][m] == max(all_scores[dataset][m] for m in ["random", "global"])] for dataset in datasets}
        ...
        for metric, metric_name in [
            ...
            score_str = " & ".join([
                f"\\small{{\\underline{{\\textbf{{{score:.1f}}}}}}}" if metric in ["random", "global"] and metric in best_unsupervised[dataset] and metric in best_overall[dataset] else
                f"\\small{{\\underline{{{score:.1f}}}}}" if metric in ["random", "global"] and metric in best_unsupervised[dataset] else
                f"\\small{{\\textbf{{{score:.1f}}}}}" if metric in best_overall[dataset] else
                f"\\small{{{score:.1f}}}"
...
compute_smoothie_latex_table(table_output_path="tables/smoothie_nlg_prompting_table.tex")
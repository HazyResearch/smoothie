...
def plot_smoothie_selection_ranks(
    datasets: List[str],
    smoothie_train: str,
    smoothie_test: str,
    title: str,
    ...
    for dataset in datasets:
        for model_group in MODEL_GROUPS:
            ...
            smoothie_train_rank = (
                sorted(ensemble_scores_list, reverse=True).index(smoothie_train_score)
                + 1
            )
            smoothie_test_rank = (
                sorted(ensemble_scores_list, reverse=True).index(smoothie_test_score)
                + 1
            )
    ...
    mean_ranks = (
        df.groupby("Dataset")[["Smoothie Train Rank", "Smoothie Test Rank"]]
        .mean()
        .reset_index()
    )
    ...
    ax.bar(
        x - width,
        mean_ranks["Smoothie Train Rank"],
        width,
        label="Smoothie Train",
        color="#4e79a7",
        alpha=0.8,
    )
    ax.bar(
        x,
        mean_ranks["Smoothie Test Rank"],
        width,
        label="Smoothie Test",
        color="#59a14f",
        alpha=0.8,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        mean_ranks["Dataset"], ha="center", fontsize=14
    )  # Increased font size
    ...
    for i, v in enumerate(mean_ranks["Smoothie Train Rank"]):
        ax.text(
            i - width, v + 0.1, f"{v:.2f}", ha="center", va="bottom", fontsize=12
        )  # Increased font size
    for i, v in enumerate(mean_ranks["Smoothie Test Rank"]):
        ax.text(
            i, v + 0.1, f"{v:.2f}", ha="center", va="bottom", fontsize=12
        )  # Increased font size
...
def generate_smoothie_selection_ranking_tables(
    datasets: List[str], smoothie_train: str, smoothie_test: str
) -> str:
    """
            ...
            data.append(
                {
                    ...
                    "train_best_rate": np.mean(
                        [r["train_is_best"] for r in dataset_results]
                    )
                    * 100,
                    "test_best_rate": np.mean(
                        [r["test_is_best"] for r in dataset_results]
                    )
                    * 100,
                    # Scores
                    ...
                    "random_score": np.mean(
                        [r["random_score"] for r in dataset_results]
                    ),
                    "oracle_score": np.mean(
                        [r["oracle_score"] for r in dataset_results]
                    ),
                }
            )
...
def save_smoothie_selection_ranking_tables(
    datasets: List[str], smoothie_train: str, smoothie_test: str, output_path: str
):
    """
    Generate and save the LaTeX tables to a file.
    """
    latex_tables = generate_smoothie_selection_ranking_tables(
        datasets, smoothie_train, smoothie_test
    )
    with open(output_path, "w") as f:
        f.write(latex_tables)


def generate_smoothie_routing_tables(
    datasets: List[str],
    smoothie_train: str,
    smoothie_test: str,
    output_path: str = "routing_results_table.tex",
) -> str:
    """
...
def create_latex_table(results_by_dataset: Dict, datasets: List[str]) -> str:
    ...
    latex_str = [
        ...
        "Dataset & "
        + " & ".join(metrics_display[m] for m in unlabeled_metrics + labeled_metrics)
        + " \\\\"
        "\\midrule",
    ]

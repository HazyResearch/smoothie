    ...
    for dataset in datasets:
        for model_group in model_groups:
            ...
            for sample_id in range(num_samples):
                scores = {
                    model: sample_scores[model][sample_id]
                    for model in ensembles[model_group]
                }
                sample_ranks.append(compute_ranks(scores))

            # Load smoothie selections
            smoothie_selections_fpath = (
                RESULTS_DIR / dataset / model_group / f"{smoothie_local}_test.json"
            )
            smoothie_weights = json.loads(smoothie_selections_fpath.read_text())[
                "smoothie_weights"
            ]
            selected_models = [
                ensembles[model_group][np.argmax(weights)]
                for weights in smoothie_weights
            ]
            selected_ranks = [
                sample_ranks[i][selected_models[i]] for i in range(num_samples)
            ]

            rank_counter = Counter(selected_ranks)
            rank_relative_freqs = [
                rank_counter[rank] / num_samples
                for rank in range(1, len(ensembles[model_group]) + 1)
            ]
            ...
            plt.title(
                f"{plot_dataset2name[dataset]} ({model_group_name})",
                pad=PLOT_TITLE_PAD,
            ...
            if plot_output_path:
                plot_fpath = (
                    ...
                    / f"{dataset}_{model_group}_rank_relative_frequency.png"
                )
...
compute_smoothie_latex_table(
    table_output_path="tables/smoothie_combined_routing_table.tex"
)

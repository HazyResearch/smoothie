...
from src.data_utils import (construct_processed_dataset_paths, generate_prompt,
                            ...
                            load_hf_dataset)
from src.utils import construct_generations_path, MODEL_GROUPS
...
def process_reference(cmp_dict: Dict[str, int], row_models_to_generations: Dict[str, str]) -> Dict[str, int]:
    """
    Process a dictionary of pairwise comparisons. Returns ranking of models.

    Args:
        cmp_dict: dict, model_a, model_b -> "A is better" or "B is better"
        row_models_to_generations: dict, model_name -> generation

    Returns:
        rank_dict: dict, model_generation -> rank
    """
    wins_dict = {}
...
def main(args):
    ...
    for i, row in train_df.iterrows():
        ...
        embedding_input = (row["instruction"].strip() + "\n\n" + row["input"].strip()).strip()
    ...
    for model in train_generations.keys():
        assert model in MODEL_GROUPS[args.model_group], f"Model {model} not in model group {args.model_group}"
    for model in MODEL_GROUPS[args.model_group]:
        assert model in train_generations.keys(), f"Model {model} not in train generations"
